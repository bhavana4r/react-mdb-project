...
export function isMonthDisabled(
  ...
  if (disablePast && currentYear === year && month < currentMonth) return true;

  if (disableFuture && currentYear === year && month > currentMonth) return true;

  if (disablePast && currentYear > year) return true;

  if (disableFuture && currentYear < year) return true;
...
export function isYearDisabled(
  ...
  if (disablePast && year < currentYear) return true;

  if (disableFuture && year > currentYear) return true;
...
export const checkPast = (date: Date, flag?: boolean) => {
  if (!flag) return false;

  const currentDate = new Date();

  if (currentDate.setHours(0, 0, 0, 0) === date.setHours(0, 0, 0, 0)) return false;
...
export const checkFuture = (date: Date, flag?: boolean) => {
  if (!flag) return false;
