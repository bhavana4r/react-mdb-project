...
import { checkFuture, checkPast, getMonth, isDateDisabled, isSameDate } from '../utils/date-utils';
...
const DatepickerDayView: React.FC<DatepickerDayProps> = ({
  filter,
  startWeekdays,
  startDay,
  inlineDayClick,
  ...
  const { min, max, setActiveDate, setSelectedDate, activeDate, selectedDate, tabCount, disableFuture, disablePast } =
            ...
            {week.map((day, dayNr) => (
              <td
                ...
                className={clsx(
                  ...
                  checkFuture(day.date, disableFuture) && 'disabled',
                  checkPast(day.date, disablePast) && 'disabled'
                )}
              >
                <div
